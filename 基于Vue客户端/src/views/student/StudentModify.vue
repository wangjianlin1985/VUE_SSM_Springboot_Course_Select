<template>
    <div id="student">
        <nav-bar class="nav">
            ...
            <template v-slot:center>编辑学生</template>
            <template v-slot:right></template>
        </nav-bar>

        <scroll class="content" top="1" bottom="50" ref="scroll" style="padding: 0px 5px">
            <van-form @submit="onSubmit">
                <van-field readonly name="student.user_name" v-model="user_name" label="手机号：" placeholder="请输入手机号" :rules="[{ required: true, message: '请填写手机号' }]" />
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            //this.user_name = this.$route.params.user_name;  //接收手机号参数
            this.user_name = localStorage.getItem("username");
            //向后端请求单条学生记录
            getStudent(this.user_name).then(res => {
                var student = res.data;
                this.user_name = student.user_name; //手机号
                this.studentNumber = student.studentNumber;  //学号
                this.password = student.password;  //登录密码
                this.studentName = student.studentName;  //姓名
                this.studentSex = student.studentSex;  //性别
                this.studentClassNumber_classNumber = student.studentClassNumber.classNumber; //所在班级
                this.studentClassNumber_className = student.studentClassNumber.className;
                this.studentBirthday = student.studentBirthday; //出生日期
                this.picker_studentBirthday = new Date(this.studentBirthday.replace(/-/g, '/'))
                this.studentState = student.studentState;  //政治面貌
                this.studentPhoto = student.studentPhoto; //学生照片
                this.studentPhotoUrl = "/" + student.studentPhoto;
                this.studentTelephone = student.studentTelephone;  //联系电话
                this.studentEmail = student.studentEmail;  //学生邮箱
                this.studentQQ = student.studentQQ;  //联系qq
                this.studentAddress = student.studentAddress;  //家庭地址
                this.studentMemo = student.studentMemo;  //附加信息
            })
