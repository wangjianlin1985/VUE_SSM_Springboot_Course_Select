<template>
    <div id="courseSelect">
        <nav-bar class="nav">
            ...
            <template v-slot:center>学生选课详情</template>
        ...
        <scroll class="content" top="1" bottom="50" ref="scroll" style="padding: 0px 5px">
            <div class="main-box">
                <div class="info-bar">
                    ...
                    <span class="title3">{{selectId}}</span>
                </div>
                <div class="info-bar">
                    <span class="title2">&nbsp;选课学生：</span>
                    <span class="title3">{{studentNumber_studentName}}</span>
                </div>
                <div class="info-bar">
                    <span class="title2">&nbsp;选择课程：</span>
                    <span class="title3">{{courseNumber_courseName}}</span>
                </div>
                <div class="info-bar">
                    <span class="title2">&nbsp;选课时间：</span>
                    <span class="title3">{{selectTime}}</span>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                selectId: "", //记录编号
                studentNumber_studentName: "", //选课学生
                courseNumber_courseName: "", //选择课程
                selectTime: "", //选课时间
            }
        },
        created() {
            this.selectId = this.$route.params.selectId; //接收到的记录编号
            //向后端请求单条学生选课记录
            getCourseSelect(this.selectId).then(res => {
                ...
                this.selectTime = courseSelect.selectTime;
            })
          document.getElementById("tabbar").style.display = "flex";
        },
        methods: {
            back() {
                //this.$router.replace('/courseSelect/list')
                this.$router.go(-1);
            },
        },
    }
