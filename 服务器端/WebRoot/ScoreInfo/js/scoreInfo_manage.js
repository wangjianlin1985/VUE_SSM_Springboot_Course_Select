...
function initScoreInfoManageTool() {
	scoreInfo_manage_tool = {
		init: function() {
			$.ajax({
				...
				success : function (data, response, status) {
					$("#studentNumber_user_name_query").combobox({ 
					    valueField:"user_name",
					    textField:"studentName",
					    panelHeight: "200px",
				        editable: false, //不允许手动输入 
					});
					data.splice(0,0,{user_name:"",studentName:"不限制"});
					$("#studentNumber_user_name_query").combobox("loadData",data); 
				}
			});
			$.ajax({
		...
		search: function() {
			...
			queryParams["studentNumber.user_name"] = $("#studentNumber_user_name_query").combobox("getValue");
				...
				$.ajax({
					...
					success : function (scoreInfo, response, status) {
						...
						if (scoreInfo) { 
							...
							$("#scoreInfo_studentNumber_user_name_edit").combobox({
								url:"Student/listAll",
							    valueField:"user_name",
							    textField:"studentName",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#scoreInfo_studentNumber_user_name_edit").combobox("select", scoreInfo.studentNumberPri);
									//var data = $("#scoreInfo_studentNumber_user_name_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#scoreInfo_studentNumber_user_name_edit").combobox("select", data[0].user_name);
						            //}
								}
							});
