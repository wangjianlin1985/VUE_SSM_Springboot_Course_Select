...
<jsp:include page="../header.jsp"></jsp:include>
	...
	<ul class="breadcrumb">
		...
		<div class="row"> 
		  ...
		  <div class="form-group">
		  	 <label for="teacher_teacherMemo_edit" class="col-md-3 text-right">教师简介:</label>
		  	 <div class="col-md-9">
			    <script name="teacher.teacherMemo" id="teacher_teacherMemo_edit" type="text/plain"   style="width:100%;height:500px;"></script>
			 </div>
		  </div>
			  <div class="form-group">
...
<script>
var teacher_teacherMemo_editor = UE.getEditor('teacher_teacherMemo_edit'); //教师简介编辑框
var basePath = "<%=basePath%>";
/*弹出修改教师界面并初始化数据*/
function teacherEdit(teacherNumber) {
  teacher_teacherMemo_editor.addListener("ready", function () {
    // editor准备好之后才可以使用 
    ajaxModifyQuery(teacherNumber);
  });
}
 function ajaxModifyQuery(teacherNumber) {
	$.ajax({
		...
		success : function (teacher, response, status) {
			if (teacher) {
				...
				teacher_teacherMemo_editor.setContent(teacher.teacherMemo, false);
			} else {
				alert("获取信息失败！");
			}
		}
