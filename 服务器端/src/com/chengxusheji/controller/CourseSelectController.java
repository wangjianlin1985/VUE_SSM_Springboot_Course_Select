...
public class CourseSelectController extends BaseController {
	...
	public void list(String selectTime,@ModelAttribute("studentNumber") Student studentNumber,@ModelAttribute("courseNumber") CourseInfo courseNumber,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (selectTime == null) selectTime = "";
		if(rows != 0)courseSelectService.setRows(rows);
		List<CourseSelect> courseSelectList = courseSelectService.queryCourseSelect(selectTime, studentNumber, courseNumber, page);
	    /*计算总的页数和总的记录数*/
	    courseSelectService.queryTotalPageAndRecordNumber(selectTime, studentNumber, courseNumber);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String selectTime,@ModelAttribute("studentNumber") Student studentNumber,@ModelAttribute("courseNumber") CourseInfo courseNumber,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (selectTime == null) selectTime = "";
		List<CourseSelect> courseSelectList = courseSelectService.queryCourseSelect(selectTime, studentNumber, courseNumber, currentPage);
	    /*计算总的页数和总的记录数*/
	    courseSelectService.queryTotalPageAndRecordNumber(selectTime, studentNumber, courseNumber);
	    /*获取到总的页码数目*/
	    int totalPage = courseSelectService.getTotalPage();
	...
	public void OutToExcel(String selectTime,@ModelAttribute("studentNumber") Student studentNumber,@ModelAttribute("courseNumber") CourseInfo courseNumber, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(selectTime == null) selectTime = "";
        List<CourseSelect> courseSelectList = courseSelectService.queryCourseSelect(selectTime,studentNumber,courseNumber);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "CourseSelect信息记录"; 
        String[] headers = { "记录编号","选课学生","选择课程","选课时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<courseSelectList.size();i++) {
        	CourseSelect courseSelect = courseSelectList.get(i); 
        	dataset.add(new String[]{courseSelect.getSelectId() + "",courseSelect.getStudentNumber().getStudentName(),courseSelect.getCourseNumber().getCourseName(),courseSelect.getSelectTime()});
        }
        /*
