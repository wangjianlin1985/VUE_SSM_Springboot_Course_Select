...
public class StudentController extends BaseController {
	...
	public void list(String user_name,String studentNumber,String studentName,@ModelAttribute("studentClassNumber") ClassInfo studentClassNumber,String studentBirthday,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public String frontlist(String user_name,String studentNumber,String studentName,@ModelAttribute("studentClassNumber") ClassInfo studentClassNumber,String studentBirthday,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (user_name == null) user_name = "";
		if (studentNumber == null) studentNumber = "";
		if (studentName == null) studentName = "";
		if (studentBirthday == null) studentBirthday = "";
		List<Student> studentList = studentService.queryStudent(user_name, studentNumber, studentName, studentClassNumber, studentBirthday, currentPage);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(user_name, studentNumber, studentName, studentClassNumber, studentBirthday);
	    /*获取到总的页码数目*/
	    int totalPage = studentService.getTotalPage();
	...
	public void OutToExcel(String user_name,String studentNumber,String studentName,@ModelAttribute("studentClassNumber") ClassInfo studentClassNumber,String studentBirthday, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(studentNumber == null) studentNumber = "";
        if(studentName == null) studentName = "";
        if(studentBirthday == null) studentBirthday = "";
        List<Student> studentList = studentService.queryStudent(user_name,studentNumber,studentName,studentClassNumber,studentBirthday);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Student信息记录"; 
        String[] headers = { "手机号","学号","姓名","性别","所在班级","出生日期","政治面貌","学生照片","联系电话","学生邮箱","联系qq"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<studentList.size();i++) {
        	Student student = studentList.get(i); 
        	dataset.add(new String[]{student.getUser_name(),student.getStudentNumber(),student.getStudentName(),student.getStudentSex(),student.getStudentClassNumber().getClassName(),student.getStudentBirthday(),student.getStudentState(),student.getStudentPhoto(),student.getStudentTelephone(),student.getStudentEmail(),student.getStudentQQ()});
        }
        /*
